...
int main(int argc, char *argv[])
{
    ...
    // a*b = c
    double a[noOfRowA][noOfColA], b[noOfColA][noOfColB], c[noOfRowA][noOfColB];
    MPI_Status status;
    ...
    if (rank == 0)
    {
        ...
        for (i = 0; i < noOfRowA; i++)
            for (j = 0; j < noOfColA; j++)
                a[i][j] = 1;

        for (i = 0; i < noOfColA; i++)
        ...
        rows_per_process = noOfRowA / numworkers;
        extra = noOfRowA % numworkers;
        ...
        for (dest = 1; dest <= numworkers; dest++)
        {
            ...
            MPI_Send(&a[offset][0], rows * noOfColA, MPI_DOUBLE, dest, tag, MPI_COMM_WORLD);
            MPI_Send(&b, noOfColA * noOfColB, MPI_DOUBLE, dest, tag, MPI_COMM_WORLD);
        ...
        for (i = 1; i <= numworkers; i++)
        {
            ...
            MPI_Recv(&c[offset][0], rows * noOfColB, MPI_DOUBLE, source, tag, MPI_COMM_WORLD, &status);
        }
    ...
    if (rank > 0)
    {
        ...
        MPI_Recv(&a, rows * noOfColA, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD, &status);
        MPI_Recv(&b, noOfColA * noOfColB, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD, &status);

        // calculate multiplication of given rows
        for (k = 0; k < noOfColB; k++)
        {
            for (i = 0; i < rows; i++)
            {
                c[i][k] = 0.0;
        ...
        MPI_Send(&c, rows * noOfColB, MPI_DOUBLE, 0, tag, MPI_COMM_WORLD);
    }
    MPI_Finalize();
}
